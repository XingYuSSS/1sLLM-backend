...
class WebServer:
    ...
    async def user_exist(self, user: str):
        """
        检查用户是否存在.
        """
        username = base64.b64decode(user).decode('utf-8')
        if self.server.check_user_name_exist(username):
            return JSONResponse(content='user_name_exist', status_code=status.HTTP_200_OK)
        return JSONResponse(content='user_name_not_exist', status_code=status.HTTP_200_OK)

    async def user_register(self, user: str, ic: str, sc: str, phone: str, pd: str):
        """
        注册.
        """
        username = base64.b64decode(user).decode('utf-8')
        if self.server.check_user_name_exist(username):
            return JSONResponse(content='user_name_exist', status_code=status.HTTP_200_OK)
        invite_code = base64.b64decode(ic).decode('utf-8')
        validation = self.invite_code_manager.validate_code(invite_code)
        if validation == 0:
            return JSONResponse(content='invalid_invite_code', status_code=status.HTTP_200_OK)
        elif validation == -1:
            return JSONResponse(content='used_invite_code', status_code=status.HTTP_200_OK)
        else:
            self.invite_code_manager.mark_code_as_used(invite_code)
        sms_code = base64.b64decode(sc).decode('utf-8')
        phone = base64.b64decode(phone).decode('utf-8')
        if sms_code != self.tmp_sms_code.get(phone):
            return JSONResponse(content='invalid_sms_code', status_code=status.HTTP_200_OK)
        del self.tmp_sms_code[phone]
        password = base64.b64decode(pd).decode('utf-8')
    ...
    async def user_login(self, request: Request, user: str, pd: str):
        """
        登录.
        """
        username = base64.b64decode(user).decode('utf-8')
        ...
        return JSONResponse(content={'success': True, 'session_id': sid}, status_code=status.HTTP_200_OK)
    ...
    async def sms_code_get(self, phone: str):
        """
        获取短信验证码.
        """
        phone = base64.b64decode(phone).decode('utf-8')
        if not re.match(r'^1[3456789]\d{9}$', phone):
            return JSONResponse(content='invalid_phone', status_code=status.HTTP_200_OK)
    ...
    async def api_providers(self):
        ...
        获取全局支持的服务商.
        """
        return JSONResponse(content=Api.get_providers(), status_code=status.HTTP_200_OK)
    ...
    async def api_list(self, request: Request):
        ...
        if user is None:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="invalid_user")
        return JSONResponse(content=user.get_api_list(), status_code=status.HTTP_200_OK)

    async def api_add(self, request: Request, name: str, key: str):
        """
        添加api.
        """
        service_provider_name = base64.b64decode(name).decode('utf-8')
        api_key = base64.b64decode(key).decode('utf-8')
        user = self.server.get_user(request.session.get('username'), request.session.get('session_id'))
        if user is None:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="invalid_user")
        if not user.add_api(service_provider_name, api_key):
            return JSONResponse(content='invalid_service_provider_name_or_key', status_code=status.HTTP_200_OK)
        return JSONResponse(content='success', status_code=status.HTTP_200_OK)

    async def api_del(self, request: Request, name: str):
        """
        删除api.
        """
        user = self.server.get_user(request.session.get('username'), request.session.get('session_id'))
    ...
    async def chat_new(self, request: Request, title: str):
        """
        新建聊天记录.
        """
        user = self.server.get_user(request.session.get('username'), request.session.get('session_id'))
        ...
        chat = data.Chat(cid, base64.b64decode(title).decode('utf-8'))
        user.add_chat(chat)
        return JSONResponse(cid, status_code=status.HTTP_200_OK)

    async def chat_get(self, request: Request, cid: str):
        """
        获取聊天记录.
        """
        user = self.server.get_user(request.session.get('username'), request.session.get('session_id'))
    ...
    async def chat_title(self, request: Request, cid: str, title: str):
        """
        更新聊天记录标题.
        """
        user = self.server.get_user(request.session.get('username'), request.session.get('session_id'))
    ...
    async def chat_del(self, request: Request, cid: str):
        """
        删除聊天记录.
        """
        user = self.server.get_user(request.session.get('username'), request.session.get('session_id'))
    ...
    async def chat_gen(self, request: Request, cid: str, p: str, provider_models: str):
        """
        生成聊天回复.
        """
        user = self.server.get_user(request.session.get('username'), request.session.get('session_id'))
        ...
        if chat is None:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="invalid_chat_id")
        prompt = base64.b64decode(p).decode('utf-8')
        if prompt is None:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="invalid_prompt")
        provider_models = json.loads(base64.b64decode(provider_models).decode('utf-8'))
    ...
    async def chat_gen_stream(self, request: Request, cid: str, p: str, provider_models: str):
        """
        流式生成聊天回复.
        """
    ...
    async def chat_sel(self, request: Request, cid: str, name: str):
        """
        选择聊天.
        """
        ...
        if chat is None:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="invalid_chat_id")
        model_name = base64.b64decode(name).decode('utf-8')
